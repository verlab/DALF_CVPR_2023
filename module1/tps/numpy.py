...
class TPS: 
    # 拟合tps变换参数n+3或者n+2，其中v对定应点的x或者y的偏移      
    @staticmethod
...
# 获得x和y的theta，拼接为（n+3）*2
def tps_theta_from_points(c_src, c_dst, reduced=False, lambd=0):
    delta = c_src - c_dst
    
    cx = np.column_stack((c_dst, delta[:, 0]))
    cy = np.column_stack((c_dst, delta[:, 1]))
        
    theta_dx = TPS.fit(cx, reduced=reduced, lambd=lambd)
    theta_dy = TPS.fit(cy, reduced=reduced, lambd=lambd)
...
#         return cv2.remap(img, mapx, mapy, cv2.INTER_CUBIC) # 重映射到新坐标点

#     import cv2
#     img = cv2.imread('/root/workspace/code/ex1/DALF_CVPR_2023/assets/1.jpg')


#     warped_front = warp_image_cv(img, c_src, c_dst, dshape=(512, 512))
#     cv2.imwrite("/root/workspace/code/ex1/DALF_CVPR_2023/assets/98.jpg",warped_front )